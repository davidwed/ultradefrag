...
void AnalyseNonResidentAttributeList(PFILENAME pfn,PMY_FILE_INFORMATION pmfi,ULONGLONG size)
{
	...
	if(size > bytes_per_cluster && (bytes_per_cluster % sizeof(ATTRIBUTE_LIST))){
		DebugPrint("Cluster size is not an integral of ATTRIBUTE_LIST size!\n");
		i = sizeof(ATTRIBUTE_LIST);
		DebugPrint("Cluster size = %I64u, ATTRIBUTE_LIST size = %u.\n",
			bytes_per_cluster,i);
		return;
	}
	
	/* allocate memory for a single cluster */
	cluster = (char *)winx_heap_alloc((SIZE_T)bytes_per_cluster);
	if(!cluster){
	...
	entries_per_cluster = bytes_per_cluster / sizeof(ATTRIBUTE_LIST);
	...
	for(block = pfn->blockmap; block != NULL; block = block->next_ptr){
		...
		for(i = 0; i < block->length; i++){
			...
			status = ReadSectors(lsn,cluster,(ULONG)bytes_per_cluster);
			if(!NT_SUCCESS(status)){
			...
			for(j = 0; j < entries_per_cluster; j++){
				...
				if(bytes_to_read < sizeof(ATTRIBUTE_LIST)){
					DebugPrint("AnalyseNonResidentAttributeList() finished,\n");
					DebugPrint("though %I64u superfluous bytes are still not processed.\n",
						bytes_to_read);
					goto scan_done;
				}
