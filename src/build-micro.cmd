@echo off
::
:: Build script for UltraDefrag Micro Edition.
::
...
if %UD_BLD_FLG_BUILD_X86% neq 0 (
	...
	"%NSISDIR%\makensis.exe" /DULTRADFGVER=%ULTRADFGVER% /DULTRADFGARCH=i386 MicroEdition.nsi || goto fail
	cd ..
)
if %UD_BLD_FLG_BUILD_AMD64% neq 0 (
	cd .\bin\amd64
	copy /Y ..\..\installer\MicroEdition.nsi .\
	"%NSISDIR%\makensis.exe" /DULTRADFGVER=%ULTRADFGVER% /DULTRADFGARCH=amd64 MicroEdition.nsi || goto fail
	cd ..\..
)
if %UD_BLD_FLG_BUILD_IA64% neq 0 (
	cd .\bin\ia64
	copy /Y ..\..\installer\MicroEdition.nsi .\
	"%NSISDIR%\makensis.exe" /DULTRADFGVER=%ULTRADFGVER% /DULTRADFGARCH=ia64 MicroEdition.nsi || goto fail
	cd ..\..
)
echo.
echo Build success!
...
if %PROCESSOR_ARCHITECTURE% == x86 (
	.\bin\ultradefrag-micro-edition-%ULTRADFGVER%.bin.i386.exe /S || goto install_fail
	goto install_success
)
if %PROCESSOR_ARCHITECTURE% == AMD64 (
	.\bin\amd64\ultradefrag-micro-edition-%ULTRADFGVER%.bin.amd64.exe /S || goto install_fail
	goto install_success
)
if %PROCESSOR_ARCHITECTURE% == IA64 (
	.\bin\ia64\ultradefrag-micro-edition-%ULTRADFGVER%.bin.ia64.exe /S || goto install_fail
	goto install_success
)
