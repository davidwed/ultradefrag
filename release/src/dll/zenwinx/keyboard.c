...
void winx_log_reg_key_info(wchar_t *key_name)
{
    ...
    DebugPrint("winx_log_reg_key_info: enumerating %ws",key_name);
    status = NtOpenKey(&hKey,KEY_READ,&oa);
    if(status != STATUS_SUCCESS){
        DebugPrintEx(status,"winx_log_reg_key_info: cannot open %ws",key_name);
        return;
    }

    status = NtQueryKey(hKey,KeyFullInformation,NULL,0,&data_size);
    if(status != STATUS_BUFFER_TOO_SMALL){
        DebugPrintEx(status,"winx_log_reg_key_info: cannot query information size");
        NtCloseSafe(hKey);
        return;
    }
    info_buffer = (KEY_FULL_INFORMATION *)winx_heap_alloc(data_size);
    if(info_buffer == NULL){
        DebugPrint("winx_log_reg_key_info: cannot allocate %u bytes of memory",data_size);
        NtCloseSafe(hKey);
        return;
    }
    ...
    if(status != STATUS_SUCCESS){
        DebugPrintEx(status,"winx_log_reg_key_info: cannot query information");
        winx_heap_free(info_buffer);
        NtCloseSafe(hKey);
        return;
    }

    DebugPrint("winx_log_reg_key_info: values count = %u",info_buffer->Values);
    DebugPrint("winx_log_reg_key_info: max value name length = %u",info_buffer->MaxValueNameLen);
    DebugPrint("winx_log_reg_key_info: max value data length = %u",info_buffer->MaxValueDataLen);

    value_name = (wchar_t *)winx_heap_alloc(info_buffer->MaxValueNameLen);
    if(value_name == NULL){
        DebugPrint("winx_log_reg_key_info: cannot allocate %u bytes of memory",info_buffer->MaxValueNameLen);
        winx_heap_free(info_buffer);
        NtCloseSafe(hKey);
        return;
    }

    value_string_data = (wchar_t *)winx_heap_alloc(info_buffer->MaxValueDataLen);
    if(value_string_data == NULL){
        DebugPrint("winx_log_reg_key_info: cannot allocate %u bytes of memory",info_buffer->MaxValueDataLen);
        winx_heap_free(value_name);
        winx_heap_free(info_buffer);
        NtCloseSafe(hKey);
        return;
    }
    ...
    for(i = 0; i < (int)info_buffer->Values; i++) {
        status = NtEnumerateValueKey(hKey,i,KeyValueFullInformation,
                NULL,0,&value_size);
        if(status != STATUS_BUFFER_TOO_SMALL){
            DebugPrintEx(status,"winx_log_reg_key_info: cannot query full information size");
            break;
        }
        value_buffer = (KEY_VALUE_FULL_INFORMATION *)winx_heap_alloc(value_size);
        if(value_buffer == NULL){
            DebugPrint("winx_log_reg_key_info: cannot allocate %u bytes of memory",value_size);
            break;
        }
        ...
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"winx_log_reg_key_info: cannot query full information");
            winx_heap_free(value_buffer);
            break;
        }
        ...
        switch(value_buffer->Type){
            case REG_DWORD:
                ...
                DebugPrint("winx_log_reg_key_info: value %d = %ws ... %d",i,value_name,value_dword_data);
                break;
            case REG_EXPAND_SZ:
            case REG_SZ:
                ...
                DebugPrint("winx_log_reg_key_info: value %d = %ws ... %ws",i,value_name,value_string_data);
                break;
        }
    ...
    winx_heap_free(value_string_data);
    winx_heap_free(value_name);
    winx_heap_free(info_buffer);
    NtCloseSafe(hKey);
}
...
static int winx_query_dev_count(wchar_t *class_name,int default_value)
{
    ...
    for(i = 0; i < sizeof(ControlSetKeys) / sizeof(wchar_t *); i++){
        ...
        InitializeObjectAttributes(&oa,&us,OBJ_CASE_INSENSITIVE,NULL,NULL);
        DebugPrint("winx_query_dev_count: checking %ws",path);
        status = NtOpenKey(&hKey,KEY_READ,&oa);
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"winx_query_dev_count: cannot open %ws",path);
            continue;
        }
        ...
        status = NtQueryValueKey(hKey,&us,KeyValuePartialInformation,
                NULL,0,&data_size);
        if(status != STATUS_BUFFER_TOO_SMALL){
            DebugPrintEx(status,"winx_query_dev_count: cannot query Count value size");
            NtCloseSafe(hKey);
            continue;
        }
        data_buffer = (KEY_VALUE_PARTIAL_INFORMATION *)winx_heap_alloc(data_size);
        if(data_buffer == NULL){
            DebugPrint("winx_query_dev_count: cannot allocate %u bytes of memory",data_size);
            NtCloseSafe(hKey);
            continue;
        }
        ...
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"winx_query_dev_count: cannot query Count value");
        ...
        if(data_buffer->Type != REG_DWORD){
            DebugPrint("winx_query_dev_count: Count value has wrong type 0x%x",
                    data_buffer->Type);
            winx_heap_free(data_buffer);
            NtCloseSafe(hKey);
            continue;
        ...
        DebugPrint("winx_query_dev_count: devices count = %u",
    ...
    DebugPrint("winx_query_dev_count: total devices count = %u",count);
    return count;
}
...
static int query_keyboard_count(void)
{
    ...
    if (winx_get_os_version() == WINDOWS_XP) {
        if (hid > 0) {
            ...
            DebugPrint("query_keyboard_count: HID above zero - "
                "keyboard count is %u (%u total)",count,total);
        } else {
            count = (total == 1 ? 2 : total);
            DebugPrint("query_keyboard_count: HID equals zero - "
                "keyboard count is %u (%u total)",count,total);
        }
    } else {
        count = total;
        DebugPrint("query_keyboard_count: not XP - keyboard "
