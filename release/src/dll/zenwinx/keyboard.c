...
static int query_keyboard_count_class(void)
{
    ...
    for(i = 0; i < 3; i++){
        ...
        DebugPrint("query_keyboard_count_class: checking %ws",us.Buffer);
        status = NtOpenKey(&hKey,KEY_READ,&oa);
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"query_keyboard_count_class: cannot open %ws",us.Buffer);
            if (i == 2)
                return kbdCount;
            else
                continue;
        ...
        if(status != STATUS_BUFFER_TOO_SMALL){
            DebugPrintEx(status,"query_keyboard_count_class: cannot query Count value size");
            NtCloseSafe(hKey);
            if (i == 2)
                return kbdCount;
            else
        ...
        if(data_buffer == NULL){
            DebugPrint("query_keyboard_count_class: cannot allocate %u bytes of memory",data_size);
            NtCloseSafe(hKey);
            if (i == 2)
                return kbdCount;
            else
        ...
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"query_keyboard_count_class: cannot query Count value");
        ...
        if(data_buffer->Type != REG_DWORD){
            DebugPrint("query_keyboard_count_class: Count value has wrong type 0x%x",
                    data_buffer->Type);
            winx_heap_free(data_buffer);
            NtCloseSafe(hKey);
            if (i == 2)
        ...
        DebugPrint("query_keyboard_count_class: old keyboard count is %u",old_kbdCount);
        DebugPrint("query_keyboard_count_class: keyboard count is %u",kbdCount);
        if (old_kbdCount > kbdCount) {
            DebugPrint("query_keyboard_count_class: using old keyboard count!");
...
static int query_keyboard_count_hid(void)
{
    ...
    for(i = 0; i < 3; i++){
        ...
        InitializeObjectAttributes(&oa,&us,OBJ_CASE_INSENSITIVE,NULL,NULL);
        DebugPrint("query_keyboard_count_hid: checking %ws",us.Buffer);
        status = NtOpenKey(&hKey,KEY_READ,&oa);
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"query_keyboard_count_hid: cannot open %ws",us.Buffer);
            if (i == 2)
                return kbdCount;
            else
                continue;
        ...
        status = NtQueryValueKey(hKey,&us,KeyValuePartialInformation,
                NULL,0,&data_size);
        if(status != STATUS_BUFFER_TOO_SMALL){
            DebugPrintEx(status,"query_keyboard_count_hid: cannot query Count value size");
            NtCloseSafe(hKey);
            if (i == 2)
                return kbdCount;
            else
        ...
        data_buffer = (KEY_VALUE_PARTIAL_INFORMATION *)winx_heap_alloc(data_size);
        if(data_buffer == NULL){
            DebugPrint("query_keyboard_count_hid: cannot allocate %u bytes of memory",data_size);
            NtCloseSafe(hKey);
            if (i == 2)
                return kbdCount;
            else
        ...
        status = NtQueryValueKey(hKey,&us,KeyValuePartialInformation,
                data_buffer,data_size,&data_size2);
        if(status != STATUS_SUCCESS){
            DebugPrintEx(status,"query_keyboard_count_hid: cannot query Count value");
            winx_heap_free(data_buffer);
            NtCloseSafe(hKey);
            if (i == 2)
                return kbdCount;
        ...
        if(data_buffer->Type != REG_DWORD){
            DebugPrint("query_keyboard_count_hid: Count value has wrong type 0x%x",
                    data_buffer->Type);
            winx_heap_free(data_buffer);
            NtCloseSafe(hKey);
            if (i == 2)
        ...
        kbdCount = (int)*(DWORD *)data_buffer->Data;
        DebugPrint("query_keyboard_count_hid: old keyboard count is %u",old_kbdCount);
        DebugPrint("query_keyboard_count_hid: keyboard count is %u",kbdCount);
        if (old_kbdCount > kbdCount) {
            DebugPrint("query_keyboard_count_hid: using old keyboard count!");
            kbdCount = old_kbdCount;
        }
        old_kbdCount = kbdCount;
...
static int query_keyboard_count(void)
{
    ...
    if (winx_get_os_version() == WINDOWS_XP) {
        if (kbdCountHID > 0) {
            ...
            DebugPrint("query_keyboard_count: HID greater zero - keyboard count is %u / %u total",kbdCount,kbdCountTotal);
        } else {
            kbdCount = (kbdCountTotal == 1 ? 2 : kbdCountTotal);
            DebugPrint("query_keyboard_count: HID zero - keyboard count is %u / %u total",kbdCount,kbdCountTotal);
        }
    } else {
        kbdCount = kbdCountTotal;
        DebugPrint("query_keyboard_count: not XP - keyboard count is %u / %u total",kbdCount,kbdCountTotal);
    }
