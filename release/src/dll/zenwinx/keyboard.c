...
static int winx_query_dev_count(wchar_t *class_name,int default_value)
{
    ...
    DebugPrint("winx_query_dev_count: devices count = %u", retrieved_count);
    ...
    if (winx_get_os_version() == WINDOWS_XP) {
        if (hid > 0) {
            ...
            DebugPrint("query_keyboard_count: HID above zero - "
                "keyboard count is %u (%u total / %u mapped)",count,total,mapped);
        } else {
        ...
        DebugPrint("query_keyboard_count: not XP - keyboard "
            "count is %u (%u total / %u mapped)",count,total,mapped);
    }
