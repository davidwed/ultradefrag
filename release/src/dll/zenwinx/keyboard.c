...
static int query_keyboard_count(void)
{
    ...
    for(i = 0; i < 3; i++){
        ...
        DebugPrint("query_keyboard_count: checking %ws",us.Buffer);
        ...
        DebugPrint("query_keyboard_count: old keyboard count is %u",old_kbdCount);
        DebugPrint("query_keyboard_count: keyboard count is %u",kbdCount);
        if (old_kbdCount > kbdCount) {
            DebugPrint("query_keyboard_count: using old keyboard count!");
